...
const API = `https://job-portal-backend-b5mj.onrender.com/job`;
...
export const jobApplyAction = (jobId, Toaster) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.post(`${API}/applyJob`, { jobId }, config);
...
export const getAppliedJobAction = () => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.get(`${API}/getAppliedjobs`, config);
...
export const updateAppliedJobAction = (id, Status,Toaster) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.patch(`${API}/updateAppliedjob/${id}`, { Status }, config);
...
export const deleteAppliedJobAction = (id) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.delete(`${API}/deleteAppliedjob/${id}`, config);
